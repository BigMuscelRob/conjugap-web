    ...
    <main className="bg-cream px-6 py-16">
      <div className="max-w-5xl w-full mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <ul className="mt-8 flex flex-col gap-4">
            {BENEFITS.map(({ emoji, text }) => (
              <li key={text} className="flex items-center gap-3">
                <span className="w-9 h-9 flex items-center justify-center rounded-full bg-saffron-50 text-[18px] shrink-0">
                  {emoji}
                </span>
                <span className="font-semibold text-ink-900 text-[15px]">{text}</span>
              </li>
            ))}
          </ul>
        ...
        <div className="bg-paper border-2 border-ink-900 rounded-[24px] p-10
          ...
          <Image
            src="/assets/mascot-wink.svg"
            width={88}
            height={88}
            alt="ConjuGap Maskottchen zwinkert"
          />
